...
import EstadoAliado from '#models/estado_aliado'

export default class EstadosAliadosController {
  async index({ response }: HttpContext) {
    const estados = await EstadoAliado.all()
    return response.ok({ estados })
  }

  async show({ params, response }: HttpContext) {
    const estado = await EstadoAliado.findOrFail(params.id)
    return response.ok({ estado })
  }

  async store({ request, response }: HttpContext) {
    const datos = request.only(['nombre'])
    const nombreExiste = await EstadoAliado.findBy('nombre', datos.nombre)
    if (nombreExiste) {
      return response.conflict({ mensaje: 'Ya existe un estado con ese nombre' })
    }
    const estado = await EstadoAliado.create({ nombre: datos.nombre })
