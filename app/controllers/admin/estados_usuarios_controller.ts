...
import EstadoUsuario from '#models/estado_usuario'

export default class EstadosUsuariosController {
  async index({ response }: HttpContext) {
    const estados = await EstadoUsuario.all()
    return response.ok({ estados })
  }

  async show({ params, response }: HttpContext) {
    const estado = await EstadoUsuario.findOrFail(params.id)
    return response.ok({ estado })
  }

  async store({ request, response }: HttpContext) {
    const datos = request.only(['nombre'])
    const nombreExiste = await EstadoUsuario.findBy('nombre', datos.nombre)
    if (nombreExiste) {
      return response.conflict({ mensaje: 'Ya existe un estado con ese nombre' })
    }
    const estado = await EstadoUsuario.create({ nombre: datos.nombre })
