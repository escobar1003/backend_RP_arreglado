...
export default class EstadosEntregasController {
  async index({ response }: HttpContext) {
    const estados = await EstadoEntrega.all()
    return response.ok({ estados })
  }

  async show({ params, response }: HttpContext) {
    const estado = await EstadoEntrega.findOrFail(params.id)
    return response.ok({ estado })
  }

  async store({ request, response }: HttpContext) {
    const datos = request.only(['nombre'])
    const nombreExiste = await EstadoEntrega.findBy('nombre', datos.nombre)
    if (nombreExiste) {
      return response.conflict({ mensaje: 'Ya existe un estado con ese nombre' })
    }
    const estado = await EstadoEntrega.create({ nombre: datos.nombre })
