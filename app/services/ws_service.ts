...
class WsService {
  //Esta clase concentra toda la lógica relacionada con los WebSockets.
  public io: Server | null = null //Es una propiedad que almacenará la instancia del servidor Socket.IO.

  public boot(httpServer: any) {
    //Este método inicializa el servidor de WebSockets.
    this.io = new Server(httpServer, {
      //Se conecta al servidor HTTP principal de AdonisJS.
      cors: {
        origin: '*',
        methods: ['GET', 'POST'],
        credentials: true,
  ...
  public emitToEncargado(idEncargado: number, event: string, data: any) {
    //Envía un evento únicamente al encargado indicado.
  ...
  public emitToUsuario(idUsuario: number, event: string, data: any) {
    //Envía un evento únicamente al usuario indicado.
